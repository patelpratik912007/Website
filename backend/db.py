...
# Vercel serverless functions have a writable but ephemeral /tmp filesystem.
# Keep production fallback data there instead of trying to write inside the
# deployed source tree. Local development continues to use backend/database/.
if os.getenv('VERCEL'):
    SQLITE_DB_DIR = '/tmp/kartik-watch-shop'
else:
...
SQLITE_SOURCE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), 'database'))
...
def ensure_sqlite_database():
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            if not table_exists:
                schema_path = os.path.join(SQLITE_SOURCE_DIR, 'sqlite_schema.sql')
                seed_path = os.path.join(SQLITE_SOURCE_DIR, 'sqlite_seed.sql')
